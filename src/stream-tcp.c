                        ...
                        StreamTcpThread *stt, TcpSession *ssn, PacketQueue *pq)
{
    ...
    /* SYN/ACK */
        ...
        if (TCP_HAS_TFO(p) && p->payload_len) {
            ssn->client.next_seq += p->payload_len;
            ssn->client.base_seq = ssn->client.isn;
            SCLogDebug("ssn: %p (TFO) isn %u base_seq %u next_seq %u",
                    ssn, ssn->client.isn, ssn->client.base_seq, ssn->client.next_seq);
